...
def run_ups(seqID, seqs, fname, show_contribution=False):
    ...
    '''
    feature, ids = generator(seqID, seqs)
    #print('The id', ids)
...
def apply_model(feature, feature_dict, y_true=None):
    ...
    '''
    scores = []
    for root, dirs, files in os.walk('./model_ups/'):
        for fl in files:
            abspath = root+fl
            #print abspath
...
def generator(ids, seqs):
    ...
    def gen_seq_feature(val, seq):
        ...
        for s in SLC:
            slc[s] = np.array(val['aaf'])[:, SLC.index(s)]
        val.update(slc)
        return val
